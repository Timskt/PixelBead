...
export default function App() {
  ...
  // Re-process when params change (brand, quality, maxColors, removeBg)
  useEffect(() => {
    if (!image) return
    doProcess(image)
  ...
  // Parameter changes — just set state, useEffect handles reprocess
  const handleBrandChange = useCallback((b: string) => setBrand(b), [])
  const handleQualityChange = useCallback((q: QualityMode) => setQuality(q), [])
  const handleMaxColorsChange = useCallback((n: number) => setMaxColors(n), [])
  const handleRemoveBgChange = useCallback((v: boolean) => setRemoveBg(v), [])
  ...
  const handleRandomSample = useCallback(async () => {
    try {
      setLoading(true)
      let idx: number
      do { idx = Math.floor(Math.random() * SAMPLE_IMAGES.length) }
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === "v") setCompareMode((p) => !p)
      if (e.key === "r") handleRandomSample()
    }
  ...
  useEffect(() => {
    const p = decodeSettings(location.hash)
    if (p.grid) setMaxGrid(Number(p.grid))
    if (p.px) setPixelSize(Number(p.px))
    if (p.q && (p.q === "fast" || p.q === "detail")) setQuality(p.q)
    if (p.colors) setMaxColors(Number(p.colors))
    if (p.bead) setBeadSize(Number(p.bead))
  ...
  const beadCounts = useMemo(
    () => result && result.width > 0 ? getBeadCounts(result.matrix) : [],
    [result]
  )
      ...
      <div className="max-w-lg mx-auto px-4 pb-8">
            ...
            <div className="grid-bg rounded-2xl overflow-hidden" style={{ maxHeight: "70vh" }}>
              <div className="flex">
                <div className="flex-1 border-r border-border">
                  ...
                  <img src={originalUrlRef.current} alt="Original" className="w-full object-contain" style={{ maxHeight: "65vh" }} />
                </div>
        ...
        <section className="mb-4 bg-bg-card rounded-2xl p-4 shadow-sm space-y-3">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <div className="flex bg-bg rounded-full p-0.5">
                <button onClick={() => handleQualityChange("fast")}
                  className={`px-4 py-1.5 text-xs font-semibold rounded-full transition-all ${quality === "fast" ? "bg-accent1 text-white shadow-sm" : "text-text-light"}`}>快速</button>
                <button onClick={() => handleQualityChange("detail")}
                  className={`px-4 py-1.5 text-xs font-semibold rounded-full transition-all ${quality === "detail" ? "bg-accent1 text-white shadow-sm" : "text-text-light"}`}>细节</button>
              </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex bg-bg rounded-full p-0.5 flex-wrap gap-0.5">
              {COLOR_LIMITS.map((c) => (
                <button key={c.value} onClick={() => handleMaxColorsChange(c.value)}
                  className={`px-2.5 py-1.5 text-xs font-semibold rounded-full transition-all ${maxColors === c.value ? "bg-primary text-white shadow-sm" : "text-text-light"}`}>{c.label}</button>
              ))}
            </div>
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex bg-bg rounded-full p-0.5 flex-wrap gap-0.5">
              {BEAD_SIZES.map((s) => (
                <button key={s.value} onClick={() => setBeadSize(s.value)}
                  className={`px-3 py-1.5 text-xs font-semibold rounded-full transition-all ${beadSize === s.value ? "bg-accent2 text-white shadow-sm" : "text-text-light"}`}>{s.label}</button>
              ))}
            </div>
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex bg-bg rounded-full p-0.5">
              <button onClick={() => setDisplayMode("dmc")}
                className={`px-4 py-1.5 text-xs font-semibold rounded-full transition-all ${displayMode === "dmc" ? "bg-primary text-white shadow-sm" : "text-text-light"}`}>DMC色号</button>
              <button onClick={() => setDisplayMode("color")}
                className={`px-4 py-1.5 text-xs font-semibold rounded-full transition-all ${displayMode === "color" ? "bg-primary text-white shadow-sm" : "text-text-light"}`}>色名</button>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex bg-bg rounded-full p-0.5 flex-wrap gap-0.5">
              <button onClick={() => handleBrandChange("无限制")}
                className={`px-3 py-1.5 text-xs font-semibold rounded-full transition-all ${brand === "无限制" ? "bg-accent2 text-white shadow-sm" : "text-text-light"}`}>无限制</button>
              {brandNames.map((b) => (
                <button key={b} onClick={() => handleBrandChange(b)}
                  className={`px-3 py-1.5 text-xs font-semibold rounded-full transition-all ${brand === b ? "bg-accent1 text-white shadow-sm" : "text-text-light"}`}>{b}</button>
              ))}
            </div>
          </div>
          ...
          {result && result.width > 0 && (
            <div className="text-xs text-text-light pt-1 space-y-1">
              ...
              <div className="flex items-center justify-between">
                ...
                <span>相邻同色: {adjPairs} 对 ({totalPixels > 0 ? Math.round(adjPairs / totalPixels * 100) : 0}%)</span>
              </div>
            </div>
          )}
        </section>
        ...
        {beadCounts.length > 0 && (
          <section className="mb-4"><BeadSummary counts={beadCounts} totalPixels={totalPixels} /></section>
        )}

        <section className="space-y-3">
          <ImageUploader onImageLoad={handleImageLoad} onError={showToast} />
          <ActionBar result={result} canvasRef={canvasRef} displayMode={displayMode} onRandomSample={handleRandomSample} showToast={showToast} />
        </section>

        <div className="mt-4 text-center text-[10px] text-text-light/30 space-x-3">
          <span>V 对比原图</span><span>R 随机示例</span><span>滚轮缩放</span><span>拖动平移</span>
        </div>
